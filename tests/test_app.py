...
def test_dashboard_happy_path(client, mock_db_connection):
    mock_db_connection.fetchall.return_value = [
        {'txn_date': date(2023, 8, 1), 'DrCr': 'Db', 'amount': 1000, 'balance': 5000, 'user_name': 'testuser'}
    ]
...
def test_generate_charts_file_cleanup(mock_remove, mock_listdir):
    transactions = [
        {'txn_date': date(2023, 8, 1), 'DrCr': 'Db', 'amount': 1000, 'balance': 5000},
        {'txn_date': date(2023, 8, 2), 'DrCr': 'Cr', 'amount': 500, 'balance': 5500}
    ]
